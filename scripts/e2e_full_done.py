...
sid = "full-" + str(int(time.time()))
d1 = run("PGDUN 的 PGSA 模块怎么实现", sid)
d2 = run("MWAT-SR 和 Dual-domain 有什么不同", sid)
report = "==== FULL DONE 1: PGDUN 的 PGSA 模块怎么实现 ====\n" + d1 + "\n\n==== FULL DONE 2: MWAT-SR 和 Dual-domain 有什么不同 ====\n" + d2
with open(r"E:/codex/GraphRAG--main/logs/e2e_full_done.txt", "w", encoding="utf-8") as f:
    f.write(report)
print("done")
